...
if (!defined("From_Index")) { echo "<p style=\"text-align: center; color: red; font-weight: bold; font-size: 150%;\">".
                               "Enter to field.php module is prohibited</p>"; return; }
...
if (isset($_POST['radAreaMode']) and $_POST['radAreaMode'] == "Ring") {
	$ContourExtUkr = "outer"; $ContourIntUkr = "inner";
...
if ($_POST['radCalcul'] != "Square") { ?><br>
	Class of reserves <?php if ($_POST['radCalcul'] == "Oil" or $_POST['radCalcul'] == "Gas") { ?> / resources <?php } ?>: 
	...
	if ($_POST['radCalcul'] == "Oil" or $_POST['radCalcul'] == "Gas") { ?> &nbsp;
		<input type="radio" name="radResCategory" value="C1z+C2" onclick="submit()"
			<?php if ($_POST['radResCategory'] == "C1z+C2") echo "checked"; ?> >C<sub>1</sub>(drainage zone)+С<sub>2</sub> &nbsp;
...
if ($_POST['cbxIHaveSquare'] == "on") { ?>
		<tr><td style="text-align: center; font-size: 120%;" colspan=20>
					Paste (Ctrl+V) or enter the area value, thousand m<sup>2</sup>:
...
<?php
		...
		<tr><td style="text-align: center; font-size: 120%;" colspan=20>
					Geometry of <?php echo $ContourExtUkr; ?> WOC / GOC:</td></tr>
		<tr>
			<td colspan=4> </td>
			<td style="font-size: 120%;" colspan=6>Scale 1: 
			...
			<td style="font-size: 120%;" colspan=10>X<sub>min</sub>, mm: 
		...
		<tr>
			<td colspan=4> </td>
			<td style="font-size: 120%;" colspan=6>Number of points: 
			...
			<td style="font-size: 120%;" colspan=10>X<sub>max</sub>, mm: 
		...
		<tr><td style="text-align: center; font-size: 120%;" colspan=20>
						Ordinates of points (bypass <?php echo $ContourExtUkr; ?> WOC / GOC clockwise), mm:</td></tr><?php 
	$iCell = 0;
	...
	if (isset($_POST['radAreaMode']) and $_POST['radAreaMode'] == "Ring") { ?>
		<tr><td style="text-align: center; font-size: 120%;" colspan=20>
					Geometry of <?php echo $ContourIntUkr; ?> WOC / GOC:</td></tr>
		<tr>
		...
		<tr><td style="text-align: center; font-size: 120%;" colspan=20>
					Ordinates of points (bypass <?php echo $ContourIntUkr; ?> WOC / GOC clockwise), mm:</td></tr><?php 
		$iCell = 0;
		for ($iRow = 0; $iRow < 5; $iRow++) { $iCellInRow = 0; ?>
		<tr><?php
			for ($iCol = 0; $iCol < 20; $iCol++) { $iCell++; $iCellInRow++; ?>
