...
if (!defined("From_Index")) { echo "<p style=\"text-align: center; color: red; font-weight: bold; font-size: 150%;\">".
                               "Enter to petroleum.php is prohibited</p>"; return; }
...
This module is under construction, to be ready next time<?php return; ?>
	<table style="margin: auto;">
		...
		<tr>
			...
			<td style="font-size: 120%;" colspan=10>&nbsp; &nbsp; Reservoir drive mechanism:
				<select style="font-size: 110%;" name="selMode" onchange="submit()" required>
					<option <?php if ($_POST['selMode'] == "water drive") echo "selected"; ?>>water drive</option>
					<option <?php if ($_POST['selMode'] == "elastic water drive") echo "selected"; ?>>elastic water drive</option>
					<option <?php if ($_POST['selMode'] == "gravity drainage drive") echo "selected"; ?>>gravity drainage drive</option>
					<option <?php if ($_POST['selMode'] == "dissolved gas drive") echo "selected"; ?>>dissolved gas drive</option>
					<option <?php if ($_POST['selMode'] == "gas drive") echo "selected"; ?>>gas drive</option>
					<option <?php if ($_POST['selMode'] == "combined drive") echo "selected"; ?>>мішаний</option>
				</select>
			</td>
		</tr><?php
...
if (!empty($_POST['sbtNext1'])) { ?>
	...
	if ($x0 >= $xk) { ?><span style="color: red; font-size: 125%">Неправильно задані абсциси точок. Виправте!</span><?php } else {
		...
		if ($NumCount < $n) { ?><span style="color: red; font-size: 125%">Введіть усі ординати точок!</span><?php } else {
			...
			// об'єм, який займають видобувні запаси нафти в пласті
			$QVExt = round($F * $Thickness * $Pority * $Saturation * $Extract / 1000, 0);
			if ($_POST['selMode'] == "dissolved gas drive"
					or $_POST['selMode'] == "gas drive"
					or $_POST['selMode'] == "combined drive") {
				$FinalGas = str_replace(',', '.', $_POST['txtFinalGas']);
				$FinalPressure = str_replace(',', '.', $_POST['txtFinalPressure']);
				$FinalZFactor = str_replace(',', '.', $_POST['txtFinalZFactor']);
				$QGasExt = round(
...
<!--
  режим покладу -> reservoir drive mechanism
  водонапірний -> water drive
  пружноводонапірний -> elastic water drive
  гравітаційний -> gravity drainage drive
  розчиненого газу -> dissolved gas drive
  газової шапки -> gas drive
  мішаний -> combined drive
-->
</p>
